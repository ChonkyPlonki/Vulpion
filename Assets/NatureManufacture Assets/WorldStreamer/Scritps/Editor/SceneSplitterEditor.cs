    ...
    public class SceneSplitterEditor : EditorWindow
    {
        ...
        void SceneSettings()
        {
            ...
            if (GUILayout.Button(new GUIContent("Generate All Scenes From Virtual Grid", "System will generate scenes and \"Collider_Stream\" prefabs for all objects at your scene that contain \"ColliderScene\" script.")))
            {
                ...
                for (int i = 0; i < sceneSplitterSettings.sceneCollectionManagers.Count; i++)
                {
                    ...
                    if (cancel)
                        break;
                    if (EditorUtility.DisplayCancelableProgressBar("Preparing Scenes " + (currentLayerID + 1) + '/' + sceneSplitterSettings.sceneCollectionManagers.Count, "Preparing scene " + layer.prefixScene, (currentLayerID / (float)sceneSplitterSettings.sceneCollectionManagers.Count)))
                    {
                        cancel = true;
                        break;
                    }
        ...
        void CreateSettings()
        {
            ...
            if (sceneSplitterSettings == null)
            {
                ...
                //Debug.Log("creating scene settings");
            }
        }
        ...
        void GenerateScenesMulti(SceneCollectionManager layer, int currentLayerID, int layersCount)
        {
            ...
            foreach (var split in mainSplits)
            {
                ...
                if (EditorUtility.DisplayCancelableProgressBar("Creating Scenes " + (currentLayerID + 1) + '/' + layersCount + " (" + layer.prefixScene + ")", "Creating scene " + Path.GetFileNameWithoutExtension(EditorSceneManager.GetActiveScene().name) + " " + i + " from " + mainSplits.Count, (currentLayerID + (i / (float)mainSplits.Count)) / (float)layersCount))
                {
                    cancel = true;
                    EditorUtility.ClearProgressBar();
                    return;
