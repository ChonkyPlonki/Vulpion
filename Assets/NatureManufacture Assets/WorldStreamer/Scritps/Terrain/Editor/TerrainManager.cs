...
namespace WorldStreamer2
{
    ...
    public class TerrainManager : Editor
    {
        ...
        public readonly GUIContent s_groupingID = EditorGUIUtility.TrTextContent("Grouping ID", "Grouping ID for auto connection");
        public readonly GUIContent s_allowAutoConnect = EditorGUIUtility.TrTextContent("Auto Connect", "Allow the current terrain tile to automatically connect to neighboring tiles sharing the same grouping ID.");
        public readonly GUIContent s_attemptReconnect = EditorGUIUtility.TrTextContent("Reconnect", "Will attempt to re-run auto connection");
        public readonly GUIContent s_drawTerrain = EditorGUIUtility.TrTextContent("Draw", "Toggle the rendering of terrain");
        public readonly GUIContent s_drawInstancedTerrain = EditorGUIUtility.TrTextContent("Draw Instanced", "Toggle terrain instancing rendering");
        public readonly GUIContent s_pixelError = EditorGUIUtility.TrTextContent("Pixel Error", "The accuracy of the mapping between the terrain maps (heightmap, textures, etc.) and the generated terrain; higher values indicate lower accuracy but lower rendering overhead.");
        public readonly GUIContent s_baseMapDist = EditorGUIUtility.TrTextContent("Base Map Dist.", "The maximum distance at which terrain textures will be displayed at full resolution. Beyond this distance, a lower resolution composite image will be used for efficiency.");
        public readonly GUIContent s_castShadows = EditorGUIUtility.TrTextContent("Cast Shadows", "Does the terrain cast shadows?");
        public readonly GUIContent s_createMaterial = EditorGUIUtility.TrTextContent("Create...", "Create a new Material asset to be used by the terrain by duplicating the current default Terrain material.");
        public readonly GUIContent s_reflectionProbes = EditorGUIUtility.TrTextContent("Reflection Probes", "How reflection probes are used on terrain. Only effective when using built-in standard material or a custom material which supports rendering with reflection.");
        public readonly GUIContent s_preserveTreePrototypeLayers = EditorGUIUtility.TrTextContent("Preserve Tree Prototype Layers", "Enable this option if you want your tree instances to take on the layer values of their prototype prefabs, rather than the terrain GameObject's layer.");

        public readonly GUIContent s_treeAndDetails = EditorGUIUtility.TrTextContent("Tree & Detail Objects");
        public readonly GUIContent s_drawTrees = EditorGUIUtility.TrTextContent("Draw", "Should trees, grass and details be drawn?");
        public readonly GUIContent s_detailObjectDistance = EditorGUIUtility.TrTextContent("Detail Distance", "The distance (from camera) beyond which details will be culled.");
        public readonly GUIContent s_detailObjectDensity = EditorGUIUtility.TrTextContent("Detail Density", "The number of detail/grass objects in a given unit of area. The value can be set lower to reduce rendering overhead.");
        public readonly GUIContent s_treeDistance = EditorGUIUtility.TrTextContent("Tree Distance", "The distance (from camera) beyond which trees will be culled. For SpeedTree trees this parameter is controlled by the LOD group settings.");
        public readonly GUIContent s_treeBillboardDistance = EditorGUIUtility.TrTextContent("Billboard Start", "The distance (from camera) at which 3D tree objects will be replaced by billboard images. For SpeedTree trees this parameter is controlled by the LOD group settings.");
        public readonly GUIContent s_treeCrossFadeLength = EditorGUIUtility.TrTextContent("Fade Length", "Distance over which trees will transition between 3D objects and billboards. For SpeedTree trees this parameter is controlled by the LOD group settings.");
        public readonly GUIContent s_treeMaximumFullLODCount = EditorGUIUtility.TrTextContent("Max Mesh Trees", "The maximum number of visible trees that will be represented as solid 3D meshes. Beyond this limit, trees will be replaced with billboards. For SpeedTree trees this parameter is controlled by the LOD group settings.");
        public readonly GUIContent s_grassWindSettings = EditorGUIUtility.TrTextContent("Wind Settings for Grass (On Terrain Data)");
        public readonly GUIContent s_wavingGrassStrength = EditorGUIUtility.TrTextContent("Speed", "The speed of the wind as it blows grass.");
        public readonly GUIContent s_wavingGrassSpeed = EditorGUIUtility.TrTextContent("Size", "The size of the 'ripples' on grassy areas as the wind blows over them.");
        public readonly GUIContent s_wavingGrassAmount = EditorGUIUtility.TrTextContent("Bending", "The degree to which grass objects are bent over by the wind.");
        public readonly GUIContent s_wavingGrassTint = EditorGUIUtility.TrTextContent("Grass Tint", "Overall color tint applied to grass objects.");

        public readonly GUIContent s_bakeLightProbesForTrees = EditorGUIUtility.TrTextContent("Bake Light Probes For Trees", "If the option is enabled, Unity will create internal light probes at the position of each tree (these probes are internal and will not affect other renderers in the scene) and apply them to tree renderers for lighting. Otherwise trees are still affected by LightProbeGroups. The option is only effective for trees that have LightProbe enabled on their prototype prefab.");
        public readonly GUIContent s_deringLightProbesForTrees = EditorGUIUtility.TrTextContent("Remove Light Probe Ringing", "When enabled, removes visible overshooting often observed as ringing on objects affected by intense lighting at the expense of reduced contrast.");
        ...
        public void OnGUI()
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.Space();
            terrainManagerSettings.terrainsDataPath = EditorGUILayout.TextField("Terrain create folder", terrainManagerSettings.terrainsDataPath);
            terrainManagerSettings.splitSize = Mathf.NextPowerOfTwo(EditorGUILayout.IntSlider("Split size", terrainManagerSettings.splitSize, 2, 32));
            terrainManagerSettings.addTerrainCulling = EditorGUILayout.Toggle("Add terrain culling", terrainManagerSettings.addTerrainCulling);
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.Space();
            terrainManagerSettings.terrainPath = EditorGUILayout.TextField("Terrain create folder", terrainManagerSettings.terrainPath);
            terrainManagerSettings.ambientLightColor = EditorGUILayout.ColorField(new GUIContent("Ambient color"), terrainManagerSettings.ambientLightColor, true, false, true);
            EditorGUILayout.Space();

            terrainManagerSettings.terrainPrefixName = EditorGUILayout.TextField("Terrain Mesh Name", terrainManagerSettings.terrainPrefixName);
            terrainManagerSettings.terrainLod = EditorGUILayout.IntSlider("Terrain lod", terrainManagerSettings.terrainLod, 0, 4);
            EditorGUILayout.Space();
            terrainManagerSettings.basemapResolution = Mathf.NextPowerOfTwo(EditorGUILayout.IntSlider("Basemap Resolution", terrainManagerSettings.basemapResolution, 128, 4096));
            terrainManagerSettings.useBaseMap = EditorGUILayout.Toggle("Use base map", terrainManagerSettings.useBaseMap);
            terrainManagerSettings.useSmoothness = EditorGUILayout.Toggle("Use smoothness map", terrainManagerSettings.useSmoothness);


            terrainManagerSettings.terrainNormalDetails = Mathf.NextPowerOfTwo(EditorGUILayout.IntSlider("Normal Details", terrainManagerSettings.terrainNormalDetails, 1, 32));
            terrainManagerSettings.terrainNormalStrength = EditorGUILayout.Slider("Normal strength", terrainManagerSettings.terrainNormalStrength, 1, 10);
            terrainManagerSettings.useTerrainNormal = EditorGUILayout.Toggle("Create normal from shape", terrainManagerSettings.useTerrainNormal);
            terrainManagerSettings.useTextureNormal = EditorGUILayout.Toggle("Create normal from textures", terrainManagerSettings.useTextureNormal);


            EditorGUILayout.Space();
            terrainManagerSettings.yOffset = EditorGUILayout.FloatField("Y offset", terrainManagerSettings.yOffset);
            terrainManagerSettings.addVerticesDown = EditorGUILayout.Toggle("Add vertices down", terrainManagerSettings.addVerticesDown);
            if (terrainManagerSettings.addVerticesDown)
                terrainManagerSettings.verticesDownDistance = EditorGUILayout.Slider("Vertices down distance", terrainManagerSettings.verticesDownDistance, 1, 100);
            ...
            EditorGUILayout.PropertyField(trainglesTreesMax, new GUIContent("Trees Max Traingles", "Trees Max Traingles to Export"));
            

            listTerrainTrees.isExpanded = EditorGUILayout.Foldout(listTerrainTrees.isExpanded, new GUIContent("Tree Prototypes", "Tree Prototypes to generate mesh lod"));
            ...
            if (listTerrainTrees.isExpanded)
            {
                ...
                for (int i = 0; i < listTerrainTrees.arraySize; i++)
                {
                    ...
                    EditorGUILayout.PropertyField(scriptable.FindPropertyRelative("active"), new GUIContent(""), GUILayout.ExpandWidth(false), GUILayout.MaxWidth(40));
        ...
        private void SceneChanged()
        {
            ...
            if (terrainManagerSettings == null)
            {
                ...
                terrainManagerSettings.materialTemplate = AssetDatabase.GetBuiltinExtraResource<Material>("Default-Terrain-Standard.mat");
        ...
        private void TerrainSettingsGUI()
        {
            ...
            terrainManagerSettings.showBasicTerrainSettings = EditorGUILayout.BeginFoldoutHeaderGroup(terrainManagerSettings.showBasicTerrainSettings, s_basicTerrain);

            if (terrainManagerSettings.showBasicTerrainSettings)
            {
                ++EditorGUI.indentLevel;

                terrainManagerSettings.groupingID = EditorGUILayout.IntField(s_groupingID, terrainManagerSettings.groupingID);
                terrainManagerSettings.allowAutoConnect = EditorGUILayout.Toggle(s_allowAutoConnect, terrainManagerSettings.allowAutoConnect);


                terrainManagerSettings.drawHeightmap = EditorGUILayout.Toggle(s_drawTerrain, terrainManagerSettings.drawHeightmap);
                terrainManagerSettings.drawInstanced = EditorGUILayout.Toggle(s_drawInstancedTerrain, terrainManagerSettings.drawInstanced);
                terrainManagerSettings.heightmapPixelError = EditorGUILayout.Slider(s_pixelError, terrainManagerSettings.heightmapPixelError, 1, 200); // former string formatting: ""
                terrainManagerSettings.basemapDistance = EditorGUILayout.Slider(s_baseMapDist, terrainManagerSettings.basemapDistance, 0.0f, 20000.0f);
                terrainManagerSettings.shadowCastingMode = (ShadowCastingMode)EditorGUILayout.EnumPopup(s_castShadows, terrainManagerSettings.shadowCastingMode);
                terrainManagerSettings.reflectionProbeUsage = (ReflectionProbeUsage)EditorGUILayout.EnumPopup(s_reflectionProbes, terrainManagerSettings.reflectionProbeUsage);
                terrainManagerSettings.materialTemplate = EditorGUILayout.ObjectField("Material", terrainManagerSettings.materialTemplate, typeof(Material), false) as Material;
            ...
            terrainManagerSettings.showTreeAndDetailSettings = EditorGUILayout.BeginFoldoutHeaderGroup(terrainManagerSettings.showTreeAndDetailSettings, s_treeAndDetails);

            if (terrainManagerSettings.showTreeAndDetailSettings)
            {
                ++EditorGUI.indentLevel;

                terrainManagerSettings.drawTreesAndFoliage = EditorGUILayout.Toggle(s_drawTrees, terrainManagerSettings.drawTreesAndFoliage);
                terrainManagerSettings.bakeLightProbesForTrees = EditorGUILayout.Toggle(s_bakeLightProbesForTrees, terrainManagerSettings.bakeLightProbesForTrees);
                terrainManagerSettings.deringLightProbesForTrees = EditorGUILayout.Toggle(s_deringLightProbesForTrees, terrainManagerSettings.deringLightProbesForTrees);

                terrainManagerSettings.preserveTreePrototypeLayers = EditorGUILayout.Toggle(s_preserveTreePrototypeLayers, terrainManagerSettings.preserveTreePrototypeLayers);
                terrainManagerSettings.detailObjectDistance = EditorGUILayout.Slider(s_detailObjectDistance, terrainManagerSettings.detailObjectDistance, 0, 250);
                terrainManagerSettings.detailObjectDensity = EditorGUILayout.Slider(s_detailObjectDensity, terrainManagerSettings.detailObjectDensity, 0.0f, 1.0f);
                terrainManagerSettings.treeDistance = EditorGUILayout.Slider(s_treeDistance, terrainManagerSettings.treeDistance, 0, 5000);
                terrainManagerSettings.treeBillboardDistance = EditorGUILayout.Slider(s_treeBillboardDistance, terrainManagerSettings.treeBillboardDistance, 5, 2000);
                terrainManagerSettings.treeCrossFadeLength = EditorGUILayout.Slider(s_treeCrossFadeLength, terrainManagerSettings.treeCrossFadeLength, 0, 200);
                terrainManagerSettings.treeMaximumFullLODCount = EditorGUILayout.IntSlider(s_treeMaximumFullLODCount, terrainManagerSettings.treeMaximumFullLODCount, 0, 10000);
            ...
            terrainManagerSettings.showGrassWindSettings = EditorGUILayout.BeginFoldoutHeaderGroup(terrainManagerSettings.showGrassWindSettings, s_grassWindSettings);

            if (terrainManagerSettings.showGrassWindSettings)
            {
                ++EditorGUI.indentLevel;

                terrainManagerSettings.wavingGrassStrength = EditorGUILayout.Slider(s_wavingGrassStrength, terrainManagerSettings.wavingGrassStrength, 0, 1);
                terrainManagerSettings.wavingGrassSpeed = EditorGUILayout.Slider(s_wavingGrassSpeed, terrainManagerSettings.wavingGrassSpeed, 0, 1);
                terrainManagerSettings.wavingGrassAmount = EditorGUILayout.Slider(s_wavingGrassAmount, terrainManagerSettings.wavingGrassAmount, 0, 1);
                terrainManagerSettings.wavingGrassTint = EditorGUILayout.ColorField(s_wavingGrassTint, terrainManagerSettings.wavingGrassTint);
        ...
        private Texture ExportBaseMap(Terrain terrainBase, string terrainName)
        {
            ...
            cam.orthographicSize = Mathf.Max((currentBounds.max.y - currentBounds.min.y) / 2.0f, (currentBounds.max.x - currentBounds.min.x) / 2.0f, (currentBounds.max.z - currentBounds.min.z) / 2.0f);

            cam.scene = previewScene;

            RenderTexture rt = new RenderTexture(terrainManagerSettings.basemapResolution, terrainManagerSettings.basemapResolution, 32);
            cam.targetTexture = rt;
            Texture2D screenShot = new Texture2D(terrainManagerSettings.basemapResolution, terrainManagerSettings.basemapResolution, TextureFormat.ARGB32, false);

            cam.Render();

            RenderTexture.active = rt;
            screenShot.ReadPixels(new Rect(0, 0, terrainManagerSettings.basemapResolution, terrainManagerSettings.basemapResolution), 0, 0);
        ...
        private Texture2D ExportTextureSmoothness(Terrain terrainBase, string terrainName)
        {
            ...
            bool drawInstanced = terrain.drawInstanced;
            terrain.drawInstanced = false;
            terrain.materialTemplate = new Material(Shader.Find("NatureManufacture Shaders/Terrain/StandardSmoothness"));
            GameObject go = terrain.gameObject;
            EditorSceneManager.MoveGameObjectToScene(go, previewScene);
            go.transform.position = Vector3.zero;
        ...
        void TerrainToMeshRim(bool allTerrains = false)
        {
            ...
            {
                EditorUtility.DisplayProgressBar("Terrain mesh generation", "Exporting terrain " + idTerrain + "/" + countTerrain + "\n Exporting mesh", idTerrain / (float)countTerrain);
                ...
                for (int x = 0; x < heightmapData.GetLength(0); x += lod)
                {
                    ...
                    for (int z = 0; z < heightmapData.GetLength(1); z += lod)
                    {
                        ...
                        positionArray[x / lod + addxz, z / lod + addxz] = new Vector4(position.x, position.y, position.z);



                        if (x == 0)
                            positionArray[0, z / lod + addxz] = new Vector4(position.x, position.y - terrainManagerSettings.verticesDownDistance, position.z);

                        if (x == heightmapData.GetLength(0) - 1)
                            positionArray[x / lod + 2, z / lod + addxz] = new Vector4(position.x, position.y - terrainManagerSettings.verticesDownDistance, position.z);

                        if (z == 0)
                            positionArray[x / lod + 1, 0] = new Vector4(position.x, position.y - terrainManagerSettings.verticesDownDistance, position.z);

                        if (z == heightmapData.GetLength(1) - 1)
                            positionArray[x / lod + addxz, z / lod + 2] = new Vector4(position.x, position.y - terrainManagerSettings.verticesDownDistance, position.z);

                        if (x == 0 && z == 0)
                            positionArray[0, 0] = new Vector4(position.x, position.y - terrainManagerSettings.verticesDownDistance, position.z);

                        if (x == 0 && z == heightmapData.GetLength(1) - 1)
                            positionArray[0, z / lod + 2] = new Vector4(position.x, position.y - terrainManagerSettings.verticesDownDistance, position.z);

                        if (x == heightmapData.GetLength(0) - 1 && z == 0)
                            positionArray[x / lod + 2, 0] = new Vector4(position.x, position.y - terrainManagerSettings.verticesDownDistance, position.z);

                        if (x == heightmapData.GetLength(0) - 1 && z == heightmapData.GetLength(1) - 1)
                            positionArray[x / lod + 2, z / lod + 2] = new Vector4(position.x, position.y - terrainManagerSettings.verticesDownDistance, position.z);
                ...
                for (int x = 0; x < positionArray.GetLength(0); x++)
                {
                    for (int z = 0; z < positionArray.GetLength(1); z++)
                    {
                        ...
                        if (x == 0)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x + 1, z].x / (float)sizeX, positionArray[x + 1, z].z / (float)sizeZ);

                        if (x == positionArray.GetLength(0) - 1)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x - 1, z].x / (float)sizeX, positionArray[x - 1, z].z / (float)sizeZ);

                        if (z == 0)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x, z + 1].x / (float)sizeX, positionArray[x, z + 1].z / (float)sizeZ);

                        if (z == positionArray.GetLength(1) - 1)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x, z - 1].x / (float)sizeX, positionArray[x, z - 1].z / (float)sizeZ);

                        if (x == 0 && z == 0)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x + 1, z + 1].x / (float)sizeX, positionArray[x + 1, z + 1].z / (float)sizeZ);

                        if (x == 0 && z == positionArray.GetLength(1) - 1)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x + 1, z - 1].x / (float)sizeX, positionArray[x, z - 1].z / (float)sizeZ);
                        ...
                        if (x == 1)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x + 2, z].x / (float)sizeX, positionArray[x + 2, z].z / (float)sizeZ);

                        if (x == positionArray.GetLength(0) - 2)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x - 2, z].x / (float)sizeX, positionArray[x - 2, z].z / (float)sizeZ);

                        if (z == 1)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x, z + 2].x / (float)sizeX, positionArray[x, z + 2].z / (float)sizeZ);

                        if (z == positionArray.GetLength(1) - 2)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x, z - 2].x / (float)sizeX, positionArray[x, z - 2].z / (float)sizeZ);

                        if (x == 1 && z == 1)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x + 2, z + 2].x / (float)sizeX, positionArray[x + 2, z + 2].z / (float)sizeZ);

                        if (x == 1 && z == positionArray.GetLength(1) - 2)
                            normal = terrainData.GetInterpolatedNormal(positionArray[x + 2, z - 2].x / (float)sizeX, positionArray[x, z - 2].z / (float)sizeZ);
                ...
                EditorUtility.DisplayProgressBar("Terrain mesh generation", "Exporting terrain " + idTerrain + "/" + countTerrain + "\n Exporting textures", (idTerrain + 0.5f) / (float)countTerrain);
                ...
                Texture basemap = ExportBaseMap(terrain, terrainName);
        ...
        void TerrainToMesh(bool exportTrees = false, bool allTerrains = false)
        {
            ...
            foreach (var terrain in terrains)
            {
                ...
                Vector3[,] positionArray = new Vector3[heightmapData.GetLength(0) / lod + (lod == 1 ? -1 : 0) + 1 + (terrainManagerSettings.addVerticesDown ? 2 : 0), heightmapData.GetLength(1) / lod + (lod == 1 ? -1 : 0) + 1 + (terrainManagerSettings.addVerticesDown ? 2 : 0)];
                ...
                meshGO.transform.position = terrain.transform.position + new Vector3(0, terrainManagerSettings.yOffset, 0);
                meshfilter.sharedMesh = meshTerrain;

                if (exportTrees)
                {
                    EditorUtility.DisplayProgressBar("Terrain mesh generation", "Exporting terrain " + idTerrain + "/" + countTerrain + "\n Exporting trees", (idTerrain + 0.75f) / (float)countTerrain);
        ...
        void FixMeshNormals(List<MeshFilter> meshFiltersToFix)
        {
            ...
            for (int i = 0; i < meshFiltersToFix.Count - 1; i++)
            {
                ...
                EditorUtility.DisplayProgressBar("Terrain mesh generation", "Normals calculating " + i + "/" + meshFiltersToFix.Count, (i) / (float)meshFiltersToFix.Count);
                ...
                for (int j = i + 1; j < meshFiltersToFix.Count; j++)
                {
                    ...
                    meshBoundsSecond.size *= 1.01f;
                    if (meshBounds.Intersects(meshBoundsSecond))
                    {
            ...
            EditorUtility.ClearProgressBar();
            //Debug.Log(EditorApplication.timeSinceStartup - editorTime);
        }
        ...
        Texture2D ExportNormalMapHeightMap(Terrain terrain, string terrainName)
        {
            ...
            for (int y = 0; y < heightMapResolution; y++)
            {
                for (int x = 0; x < heightMapResolution; x++)
                {
                    ...
                    rawHeights[y, x] = 1 - (rawHeights[y, x] - min) / (float)(max - min);
                }
            }
            ...
            Texture2D normalTextures = ExportTextureNormalmap(terrain, terrainName);
            ...
            for (int x = 0; x < normalTextures.width; x++)
            {
                for (int y = 0; y < normalTextures.height; y++)
                {
                    ...
                    {
                        Vector4 colorNormal = normalTerrain.GetPixel(Mathf.FloorToInt(x / resize), Mathf.FloorToInt(y / resize));
                    ...
                    if (terrainManagerSettings.useTerrainNormal && !terrainManagerSettings.useTextureNormal)
                    {
                        colorNormalTexture = normalTerrain.GetPixel(Mathf.FloorToInt(x / resize), Mathf.FloorToInt(y / resize));
                    }
            ...
            byte[] pngData = normalFinal.EncodeToPNG();// GetNormalMap(rawHeights, 20 * terrainManagerSettings.terrainNormalStrength).EncodeToPNG();
        ...
        Texture2D GetNormalMap(float[,] rawHeights, float str = 2.0f)
        {
            ...
            Texture2D mipTexture = Resize(normal, normal.width / terrainManagerSettings.terrainNormalDetails, normal.width / terrainManagerSettings.terrainNormalDetails);
        ...
        void SplitTerrain(bool allTerrains = false)
        {
            ...
            foreach (var terrain in terrains)
            {
                ...
                var progressCaption = "Spliting terrain " + terrain.name + " (" + currentObjectIndex.ToString() + " of " + terrains.Length.ToString() + ")";
                ...
                for (int i = 0; i < terrainManagerSettings.terrainsCount; i++)
                {
                    ...
                    AssetDatabase.CreateAsset(td, "Assets" + terrainManagerSettings.terrainsDataPath + newTerrain.name + ".asset");
                    ...
                    float[,] terrainHeight = terrain.terrainData.GetHeights(0, 0, terrain.terrainData.heightmapResolution, terrain.terrainData.heightmapResolution);

                    float[,] partHeight = new float[terrain.terrainData.heightmapResolution / terrainManagerSettings.splitSize + 1, terrain.terrainData.heightmapResolution / terrainManagerSettings.splitSize + 1];
                    ...
                    for (int x = startX; x < end; x++)
                    {
                        ...
                        bool cancel = EditorUtility.DisplayCancelableProgressBar(progressCaption, progressText + " (Split height)", progress + (((float)x / (end - startX)) / 4f) / (float)terrainManagerSettings.terrainsCount);
                    ...
                    float[,,] terrainSplat = terrain.terrainData.GetAlphamaps(0, 0, terrain.terrainData.alphamapResolution, terrain.terrainData.alphamapResolution);

                    float[,,] partSplat = new float[terrain.terrainData.alphamapResolution / terrainManagerSettings.splitSize,
                    ...
                    {
                        bool cancel = EditorUtility.DisplayCancelableProgressBar(progressCaption, progressText + " (Processing splat " + s + ")", progress + (1 / 4f + (s / (float)terrain.terrainData.alphamapLayers) / 4f) / (float)terrainManagerSettings.terrainsCount);
                        if (cancel)
                        {
                    ...
                    td.SetDetailResolution(terrain.terrainData.detailResolution / terrainManagerSettings.splitSize, terrain.terrainData.detailResolutionPerPatch);
                    ...
                    {
                        bool cancel = EditorUtility.DisplayCancelableProgressBar(progressCaption, progressText + " (Processing detail " + d + ")", progress + (2 / 4f + (d / (float)terrain.terrainData.detailPrototypes.Length) / 4f) / (float)terrainManagerSettings.terrainsCount);
                        if (cancel)
                        {
                            Undo.CollapseUndoOperations(Undo.GetCurrentGroup());
                            EditorUtility.ClearProgressBar();
                        ...
                        int[,] terrainDetail = terrain.terrainData.GetDetailLayer(0, 0, terrain.terrainData.detailResolution, terrain.terrainData.detailResolution, d);
                    ...
                    for (int t = 0; t < treeInstances.Length; t++)
                    {
                        if (t % 1000 == 0)
                        {
                            bool cancel = EditorUtility.DisplayCancelableProgressBar(progressCaption, progressText + " (Processing trees " + t + "/" + treeInstances.Length + ")", progress + (3 / 4f + ((float)t / treeInstances.Length) / 4f) / (float)terrainManagerSettings.terrainsCount);
                            if (cancel)
                            {
                                Undo.CollapseUndoOperations(Undo.GetCurrentGroup());
                                EditorUtility.ClearProgressBar();
                             ...
                             ti.position.z >= sizeCheckX && ti.position.z <= sizeCheckX1)
                        {
                            ...
                            ti.position = new Vector3((ti.position.x * terrainManagerSettings.splitSize) % 1, ti.position.y, (ti.position.z * terrainManagerSettings.splitSize) % 1);
        ...
        public List<GameObject> ExportTrees(Terrain _terrain)
        {
            ...
            for (int tID = 0; tID < data.treePrototypes.Length; tID++)
            {
                ...
                foreach (TreeInstance tree in data.treeInstances)
                {
                    ...
                    {
                        Vector3 position = new Vector3(tree.position.x * width, tree.position.y * y, tree.position.z * height);// + _terrain.transform.position;
                        ...
                        if (treeInRange)
                        {
                            ...
                            treeG.transform.localScale = new Vector3(tree.widthScale, tree.heightScale, tree.widthScale);
                ...
                if (trees[tID] != null)
                {
                    ...
                    name = terrainManagerSettings.terrainPath + data.treePrototypes[tID].prefab.name + "_" + treeMaterial.name + ".mat";
                    pathMesh = "Assets/" + name;
                    ...
                    while (i < meshFilters.Count)
                    {
                        ...
                        if (meshVertsCount > terrainManagerSettings.trainglesTreesMax)
                        {
                            ...
                            GameObject parent = new GameObject(_terrain.name + "_Tree_Batch_" + data.treePrototypes[tID].prefab.name + "_" + id);
